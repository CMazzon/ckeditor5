...
import MathEditor from '../src/ckeditor';
...
describe( 'ClassicEditor build', () => {
	...
	describe( 'build', () => {
		it( 'contains plugins', () => {
			expect( MathEditor.builtinPlugins ).to.not.be.empty;
		} );

		it( 'contains config', () => {
			expect( MathEditor.defaultConfig.toolbar ).to.not.be.empty;
		} );
	} );

	describe( 'create()', () => {
		beforeEach( () => {
			return MathEditor.create( editorElement )
				.then( newEditor => {
					editor = newEditor;
				} );
		} );
		...
		it( 'creates an instance which inherits from the ClassicEditor', () => {
			expect( editor ).to.be.instanceof( MathEditor );
	...
	describeMemoryUsage( () => {
		testMemoryUsage(
			...
			() => MathEditor.create( document.querySelector( '#mem-editor' ) ) );
	} );
} );
